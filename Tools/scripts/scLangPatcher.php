...
if ( (isset($argv[1])) && ( (boolval(trim($argv[1])) == true) || (boolval(trim($argv[1])) == false) )) $logCopyPasteFormat = boolval(trim($argv[1]));
if ( (isset($argv[2])) && ( (boolval(trim($argv[2])) == true) || (boolval(trim($argv[2])) == false) )) $onlyLogs = boolval(trim($argv[2]));
...
if (trim($archivoOriginal) == "") {echo (getTranslation("original.file.name.blank")); die();}
if (trim($archivoNuevo) == "") {echo (getTranslation("new.file.name.blank")); die();}
if (trim($archivoFinal) == "") {echo (getTranslation("final.file.name.blank")); die();}
if ($crearLogs == true && trim($archivoLog) == "") {echo (getTranslation("log.file.name.blank")); die();}
if ($debugLevel != 1 && $debugLevel != 2 && $debugLevel != 3 && $debugLevel != 4) {echo (getTranslation("debug.level.invalid")); die();}

try
{
    if (!file_exists(__DIR__ . "/" . $archivoOriginal)) throw new Exception;
...
try
{
    if (!file_exists(__DIR__ . "/" . $archivoNuevo)) throw new Exception;
...
if ($debugLevel == 1 || $debugLevel == 2 || $debugLevel == 3 || $debugLevel == 4) echo "\n".getTranslation("searching.new.lines")."\n";
...
if ($crearLogs)
{
    ...
    {
        if ($logCopyPasteFormat == false) $logContent = $logContent."[".$key."] => ".$value."\n";
        elseif ($logCopyPasteFormat == true) $logContent = $logContent.$key."=".$value;
    }
}
...
foreach ($originalFileAsociativeArray as $key => $value)
{
    ...
    if ($existe == false) $deletedLines[$key] = $value;
}
...
if ($crearLogs) file_put_contents($logFile , $logContent);
...
if ($onlyLogs == false)
{
    ...
    {
        $counter++;
        if ($debugLevel == 2 || $debugLevel == 3) getLineInterval();
        elseif ($debugLevel == 4) echo ("\n".getTranslation("generating.line").": ".$counter);
